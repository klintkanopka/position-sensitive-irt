...
class IRTP(nn.Module):
    def __init__(self, n_persons, n_items, n_params):
        ...
        self.beta_e = nn.Parameter(torch.zeros(self.n_items) - offset/2)
        self.beta_l = nn.Parameter(torch.zeros(self.n_items) + offset/2)
    ...
    def forward(self, X):
        ...
        # fix the variance of the theta distribution to be one for a 2PL kernel
        if self.n_params < 2:
            theta_scale = 1
        else:
            theta_scale = self.theta.std()

        beta_mean = torch.cat((self.beta_e, self.beta_l)).mean()

        p_e = self.sigmoid(self.alpha_e[i_idx] * (self.theta[p_idx]/theta_scale - (self.beta_e[i_idx] - beta_mean)))
        p_l = self.sigmoid(self.alpha_l[i_idx] * (self.theta[p_idx]/theta_scale - (self.beta_l[i_idx] - beta_mean)))
